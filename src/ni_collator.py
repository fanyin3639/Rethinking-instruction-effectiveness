# Modified from NIv2, we add our ablation study part and metadata part
import logging
import random
import string
import json
...
class DataCollatorForNI:
    ...
    use_meta_data: bool = False
    do_analysis: bool = True
    do_ablation: bool = True
    ablate_content: str = 'Additional'
    meta_and_definition: bool = False
        ...
        for instance in batch:
            ...
            if add_task_definition:
                if self.use_meta_data:
                    ...
                    with open(self.label_space_path, 'r') as f:
                        desp_list = json.load(f)
                    label_space = desp_list[f'{instance["Task"]}.json']['demo labels']
                    definition = f"Category: {', '.join(category)}.\nReasoning: {', '.join(reasoning)}.\nDomains: {', '.join(domains)}.\nLabel space: {', '.join(label_space)}"
                    if self.meta_and_definition:
                        definition += "\n"
                    ...
                    if self.do_ablation:
                        text = instance["Definition"][0] if isinstance(instance["Definition"], list) else instance[
                            "Definition"]
                        with open(self.annotation_path, 'r') as f:
                            definition_df = json.load(f)
                        sent_list = definition_df[text][:-1]
                        text = ''
                        for sent in sent_list:
                            if self.ablate_content not in sent[1]:
                                text += sent[0]
                                text += ' '
            ...
            for idx, pos_example in enumerate(instance["Positive Examples"][:num_pos_examples]):
                ...
                # pos_example_str += f" Output: {'support' if 'Yes.' in pos_example['output'].strip() else 'insufficient'}"
                if not pos_example_str[-1] in string.punctuation:
                    pos_example_str += "."
                pos_example_str += "\n"
