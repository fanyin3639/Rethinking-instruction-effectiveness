...
# We modify from the codebase NIv2
...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, cl_dir=None, continual_learning=None, task_dir=None, max_num_instances_per_task=None, max_num_instances_per_eval_task=None, specific_task=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.task_dir: str = task_dir
        self.specific_task: str = specific_task
        self.cl_dir: str = cl_dir
        self.continual_learning: str = continual_learning
        ...
        return [
            ...
            datasets.SplitGenerator(
                ...
                gen_kwargs={
                    ...
                    "max_num_instances_per_task": self.config.max_num_instances_per_eval_task,
                    "subset": "test",
                    "specific_task": self.config.specific_task
                }),
        ]

    def _generate_examples(self, path=None, task_dir=None, max_num_instances_per_task=None, subset=None, specific_task=None):
        """Yields examples."""
        logger.info(f"Generating tasks from = {path}")
        generated_task = []
        with open(path, encoding="utf-8") as split_f:
            for line in split_f:
                generated_task.append(line.strip())
        for line in generated_task:
            task_name = line.strip()
            if specific_task is not None:
                if not task_name == specific_task:
                    continue
            ...
            with open(task_path, encoding="utf-8") as task_f:
                ...
                if subset == "test":
                    ...
                    # if this is the 119 test tasks, use first 100 examples, otherwise use 100-200, which is also balanecd
                    # so, we use them here
