...
# Above are original comments from Super-NIv2. We modify from their codebase
...
os.environ['WANDB_CACHE_DIR'] = '/local2/fanyin' 
try:
...
class DataTrainingArguments:
    ...
    cl_dir: Optional[List[str]] = field(
        default=None, metadata={"help": "Continual learning datasets"}
    )
    ...
    annotation_path: Optional[str] = field(
        default='',
        metadata={"help": "the path to where you store the annotation for content type. Now it is in the analysis_util folder"}
    )
    label_space_path: Optional[str] = field(
        default='',
        metadata={"help": "the path to where you store the label space information. Now it is in the analysis_util folder"}
    )
    ablate_content: Optional[str] = field(
        default='Additional',
        metadata={"help": "the content type that you want to ablate out in ablation study"}
    )        
    use_meta_data: Optional[bool] = field(
        default=False,
        metadata={"help": "use meta data and label space info"}
    )
    do_analysis: Optional[bool] = field(
        default=False,
        metadata={"help": "Definition analysis"}
    )
    do_ablation: Optional[bool] = field(
        default=False,
        metadata={"help": "Definition ablation"}
    )
    meta_and_definition: Optional[bool] = field(
        default=False,
        metadata={"help": "Definition + meta"}
    )
...
def main():
    ...
    data_collator = DataCollatorForNI(
        ...
        use_meta_data=data_args.use_meta_data,
        ablate_content=data_args.ablate_content,
        label_space_path=data_args.label_space_path,
        annotation_path=data_args.annotation_path,
        do_analysis=data_args.do_analysis,
        do_ablation=data_args.do_ablation,
        meta_and_definition=data_args.meta_and_definition
    )
